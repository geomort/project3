...
def plot_random_predictions(model, dataset, num_samples, path, time, number_images, 
                            rows=None, cols=None, class_names=None, device="cpu", show_prob=True, save_figures=True, show_plot=True):
    """
    Docstring created by Copilot:

    Displays random predictions from a trained model on a given dataset.

    This function randomly selects samples from the dataset, runs them through the model,
    and visualizes the predicted vs. true labels. It optionally shows prediction confidence
    and colors titles based on correctness (green for correct, red for incorrect).

    Args:
        model (torch.nn.Module): Trained PyTorch model for inference.
        dataset (torch.utils.data.Dataset): Dataset providing (image, label) pairs.
        ...
        rows (int, optional): Number of rows in the plot grid. If None, computed dynamically.
        cols (int, optional): Number of columns in the plot grid. If None, computed dynamically.
        class_names (list[str], optional): List of class names. If None, attempts to use `dataset.classes`.
        device (str, optional): Device for inference ('cpu' or 'cuda'). Default is 'cpu'.
        show_prob (bool, optional): Whether to display prediction confidence in the title. Default is True.
        save_figures (bool, optional): Whether to save the figure to disk. Default is True.
        show_plot (bool, optional): Whether to display the plot interactively. Default is True.

    Behavior:
        - Randomly selects `num_samples` images from the dataset.
        - Performs inference and computes prediction probabilities.
        - Displays images in a grid with predicted and true labels.
        - Colors titles green if prediction is correct, red otherwise.
        - Saves the figure as a PNG file if `save_figures` is True.
        - Shows or closes the plot based on `show_plot`.

    Notes:
        - Requires the model to output raw logits for classification.
        - If `class_names` is not provided and `dataset` lacks `.classes`, raises ValueError.
        - Images are displayed in their original color format.

    Example:
        plot_random_predictions(model, test_dataset, num_samples=12, path=Path('./plots'),
                                time='2025-11-18', number_images=100, class_names=['cat', 'dog'],
                                device='cuda', show_prob=True, save_figures=True, show_plot=True)
    """
    ...
    plt.tight_layout()
    fig.subplots_adjust(top=0.85)  # Reserve space for suptitle
    plt.suptitle('Predictions and true labels')
    if save_figures: plt.savefig(path / f'{time}_Predictions_and_true_number_images{number_images}_samples{num_samples}.png', dpi=300, bbox_inches='tight')
    if show_plot:
        plt.show()
    else:
        plt.close()